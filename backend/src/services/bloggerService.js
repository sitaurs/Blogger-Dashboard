...
class BloggerService {
  ...
  async getBlogs(userId = null) {
    ...
    try {
      ...
      // Try to return cached blogs if API fails
      const cachedBlogs = await Blog.findByUser(targetUserId);
      if (cachedBlogs.length > 0) {
        console.log('📦 Returning cached blogs due to API error');
        return cachedBlogs.map(blog => blog.toSummary());
      }
  ...
  async getBlog(blogId, userId = null) {
    ...
    try {
      ...
      // Try cached version
      const cachedBlog = await Blog.findOne({ blogId, userId: targetUserId });
      if (cachedBlog) {
        return cachedBlog.toSummary();
      }
  ...
  async getPosts(blogId, options = {}, userId = null) {
    ...
    try {
      ...
      // Return cached posts if API fails
      const cachedPosts = await Post.findByUserAndBlog(targetUserId, blogId, {
  ...
  async getPost(blogId, postId, userId = null) {
    ...
    try {
      ...
      // Try cached version
      const cachedPost = await Post.findOne({ postId, userId: targetUserId });
      if (cachedPost) {
        return cachedPost;
      }
